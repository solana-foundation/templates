...
describe('solana-distributor', () => {
  ...
  before(async () => {
    ...
    ;[airdropStateKey] = PublicKey.findProgramAddressSync([Buffer.from('merkle_tree')], program.programId)
  ...
  it('Claim SOL - Recipient 1', async () => {
    ...
    const [claimStatusKey] = PublicKey.findProgramAddressSync(
      [Buffer.from('claim'), airdropStateKey.toBuffer(), recipient1.publicKey.toBuffer()],
      program.programId,
    )
  ...
  it('Claim SOL - Recipient 2', async () => {
    ...
    // Derive claim status PDA
    const [claimStatusKey] = PublicKey.findProgramAddressSync(
      [Buffer.from('claim'), airdropStateKey.toBuffer(), recipient2.publicKey.toBuffer()],
      program.programId,
    )
