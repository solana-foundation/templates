...
function ConnectButton() {
  ...
  if (user || solanaWallet) {
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={async () => { try { await logout() } catch (e) { console.error('Logout failed:', e) } finally { window.location.href = '/' } }}
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <>
          <div className="fixed inset-0 bg-black/20 z-40" onClick={closeModal} />
          <div
          ...
          >
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={closeModal} className="text-[#606060] hover:text-[#030303] text-lg leading-none" aria-label="Close">
                ×
              </button>
            </div>

            {error && (
              <div className="mb-4 p-3 rounded-lg text-sm text-[#C5221F] bg-[#FCE8E6]" style={{ border: '1px solid #F5C6C2' }}>
                {error}
              </div>
            )}
              ...
              {emailStep === 'input' ? (
                <form onSubmit={handleSendOtp} className="flex gap-2">
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                    placeholder="your@email.com" required className={inputClass} style={{ border: '1px solid #DADADA' }} />
                  <button type="submit" disabled={isSendingOtp || !email}
                ...
                <form onSubmit={handleVerifyOtp} className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <input type="text" value={otpCode} onChange={(e) => setOtpCode(e.target.value)}
                      placeholder="6-digit code" required autoFocus className={inputClass} style={{ border: '1px solid #DADADA' }} />
                    <button type="submit" disabled={isVerifyingOtp || !otpCode}
                      className="px-3 py-2 bg-[#4779FF] hover:bg-[#3366ee] disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm rounded-lg font-medium transition-colors whitespace-nowrap">
                      {isVerifyingOtp ? '...' : 'Verify'}
                    </button>
                  </div>
                  <button type="button" onClick={() => { setEmailStep('input'); setOtpCode('') }}
              ...
              <>
                ...
                <div className="space-y-1">
                  {solanaProviders.map((provider) => (
                    <button key={provider.key} onClick={() => handleWalletConnect(provider.key)}
                      ...
                      {provider.metadata.icon && <img src={provider.metadata.icon} alt="" className="w-5 h-5 rounded" />}
                      <span className="flex-1 text-left">{provider.metadata.displayName}</span>
                      {isConnectingWallet === provider.key && <span className="text-xs text-[#606060]">Connecting...</span>}
                    </button>
                  ))}
                </div>
              </>
