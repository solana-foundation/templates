...
interface PositionFormProps {
  ...
  isLoading: boolean
  onOpenPosition: (params: { direction: 'long' | 'short'; marketIndex: number; baseAmount: string }) => void
  txState: DriftTransactionState
}

export function PositionForm({ isInitialized, isLoading, onOpenPosition, txState }: PositionFormProps) {
  const [direction, setDirection] = useState<'long' | 'short'>('long')
  const [baseAmount, setBaseAmount] = useState(DEFAULT_BASE_SIZE)

  const isBusy = isLoading || txState.status === 'pending' || txState.status === 'signing'
      ...
      <div className="flex flex-col gap-5">
        <div>
          ...
          <p className="mt-2 text-sm text-muted">
            This demo places a market order on Drift devnet and confirms the transaction before updating state.
          </p>
        </div>
        ...
        <button
          ...
          onClick={() => onOpenPosition({ direction, marketIndex: DEFAULT_MARKET_INDEX, baseAmount })}
        ...
        {txState.action === 'open' && txState.status !== 'idle' ? (
          <div className="rounded-2xl border border-border-low bg-card/70 px-4 py-3 text-sm text-muted">
            ...
            {txState.status === 'pending' && 'Transaction submitted. Confirming on-chain...'}
            {txState.status === 'confirmed' && 'Position opened successfully.'}
            {txState.status === 'error' && txState.error}
            {txState.signature ? (
              <p className="mt-2 break-all font-mono text-xs text-foreground">{txState.signature}</p>
            ) : null}
          </div>
        ) : null}
      </div>
