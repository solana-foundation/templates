...
export function WalletCard({ isInitialized, isLoading, onInitialize, txMessage }: WalletCardProps) {
  return (
    <section className="glass-panel fade-in rounded-[var(--radius)] p-6">
      <div className="flex flex-col gap-4">
        ...
        >
          {isInitialized ? 'Account ready' : isLoading ? 'Connecting...' : 'Initialize Drift Account'}
        </button>
        {txMessage ? (
          <div className="rounded-2xl border border-border-low bg-card/70 px-4 py-3 text-sm text-muted">
            {txMessage}
