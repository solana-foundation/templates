...
import { BASE_DECIMALS, DEFAULT_MARKET_INDEX, QUOTE_DECIMALS } from '@/lib/drift'
...
export function useDrift() {
  ...
  const [txState, setTxState] = useState<DriftTransactionState>({ action: null, status: 'idle' })
  ...
  useEffect(() => {
    ...
    let cancelled = false
    const accountLoader = new sdk.BulkAccountLoader(connection, 'confirmed', 1000)
  ...
  const initializeUserAccount = useCallback(async () => {
    ...
    try {
      ...
      setTxState({
        action: 'init',
        status: 'error',
        error: error instanceof Error ? error.message : 'Failed to initialize Drift account.',
      })
    }
  ...
  const depositSolCollateral = useCallback(
    async (amount: string) => {
      ...
      if (depositAmount.lte(new sdk.BN(0))) {
        setTxState({ action: 'deposit', status: 'error', error: 'Enter a SOL amount greater than 0.' })
        return
      }

      setTxState({ action: 'deposit', status: 'signing' })
      try {
        const signature = await driftClient.deposit(depositAmount, 1, wallet.publicKey, SUB_ACCOUNT_ID)
        const { blockhash, lastValidBlockHeight } = await connection.getLatestBlockhash()
    ...
    [connection, driftClient, refreshUserAccount, sdk, userAccount, wallet?.publicKey]
  )

  const openPosition = useCallback(
    async (params: { direction: 'long' | 'short'; marketIndex: number; baseAmount: string }) => {
      if (!driftClient || !sdk || !wallet?.publicKey) {
        setTxState({
          action: 'open',
          status: 'error',
      ...
      const baseAssetAmount = new sdk.BN(Number.isFinite(parsed) ? Math.round(parsed * basePrecision) : 0)
      if (baseAssetAmount.lte(new sdk.BN(0))) {
        setTxState({ action: 'open', status: 'error', error: 'Enter a base size greater than 0.' })
        return
      }
      ...
      try {
        ...
        setTxState({
          action: 'open',
          status: 'error',
          error: isCollateralError ? 'Insufficient collateral. Deposit SOL before trading.' : message,
        })
      }
    },
    [connection, driftClient, refreshUserAccount, sdk, userAccount, wallet?.publicKey]
  ...
  const positionView = useMemo<DriftPositionView | null>(() => {
    ...
    const position = userAccount.perpPositions.find((perpPosition) => perpPosition.marketIndex === DEFAULT_MARKET_INDEX)
    ...
    return {
      ...
      baseAssetAmount: (Number(baseAssetAmount.toString()) / 10 ** BASE_DECIMALS).toFixed(6),
      quoteAssetAmount: (Number(quoteAssetAmount.abs().toString()) / 10 ** QUOTE_DECIMALS).toFixed(2),
      direction,
    }
  }, [sdk, userAccount])
