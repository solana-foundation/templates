...
export default function Page() {
  const { isInitialized, isLoading, positionView, txState, initializeUserAccount, depositSolCollateral, openPosition } =
    useDrift()
    ...
    txState.action === 'init'
      ? txState.status === 'signing'
        ...
        : txState.status === 'pending'
          ? 'Creating Drift account on-chain...'
          : txState.status === 'confirmed'
            ? 'Drift account created.'
            : txState.status === 'error'
              ? txState.error ?? 'Account initialization failed.'
              : null
      : null
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-10">
        <header className="fade-in space-y-4">
          ...
          <p className="max-w-2xl text-base text-muted md:text-lg">
            Connect a wallet, initialize your Drift account PDA, then open a simple perp position on devnet.
            Everything is wired with the Drift SDK, wallet adapter, and real transaction confirmations.
          </p>
        </header>
        ...
        <section className="fade-in rounded-[var(--radius)] border border-border-low bg-card/50 px-6 py-5 text-sm text-muted">
          ...
          <p className="mt-2">
            Drift devnet uses real program IDs and market data. Make sure your wallet has devnet SOL and
            consider using a dedicated devnet wallet for testing.
          </p>
        </section>
      </div>
    </main>
