...
describe('staking-program', () => {
  ...
  const REWARD_RATE = 100 // 100 tokens per second
  ...
  it('Fails to unstake more than staked amount', async () => {
    try {
      await program.methods
        .unstake(new anchor.BN(STAKE_AMOUNT + 1))
        .accounts({
          signer: admin.publicKey,
          userTokenAccount: userTokenAccount,
          vault: vaultPda,
  ...
  it('Unstakes tokens', async () => {
    ...
    // Verify staker state
    const staker = await program.account.staker.fetch(stakerPda)
    assert.equal(staker.stakedAmount.toNumber(), STAKE_AMOUNT - unstakeAmount)

    // Verify global state
    const globalState = await program.account.globalState.fetch(globalStatePda)
    assert.equal(globalState.totalStaked.toNumber(), STAKE_AMOUNT - unstakeAmount)
  })
})
