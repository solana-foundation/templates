...
2. Whitelist your redirect URL: `http://localhost:5173/` (for development)
3. Add your App ID to `.env` file

**Important**: Keep your App ID secure and never commit `.env` to version control.
...
3. User is redirected back with authenticated session
4. App displays wallet address and SOL balance
...
| `VITE_REDIRECT_URL`     | Yes      | -                               | OAuth redirect URL (must be whitelisted) |
| `VITE_PHANTOM_AUTH_URL` | No       | `https://connect.phantom.app`   | Phantom Connect URL                      |
| `VITE_SOLANA_RPC_URL`   | No       | `https://api.devnet.solana.com` | Solana RPC endpoint                      |
...
Ensure your redirect URL `http://localhost:5173/` is whitelisted in Phantom Portal. URL must match exactly (including trailing slash).
...
**Recommended RPC Providers**: [Helius](https://helius.dev), [Alchemy](https://www.alchemy.com/solana), [QuickNode](https://www.quicknode.com/chains/sol)
...
- Update `VITE_REDIRECT_URL` to your production URL
- Whitelist production URL in Phantom Portal
- Use a reliable Solana RPC endpoint (Helius, Alchemy, or QuickNode)
