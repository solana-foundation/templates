import { initializeSDK, connect, disconnect, getAddress, checkExistingSession, handleAuthCallback } from './phantom'
...
async function initializeApp(): Promise<void> {
  try {
    ...
    const hasAuthParams =
      urlParams.has('code') || urlParams.has('state') || urlParams.has('wallet_id') || urlParams.has('response_type')

    if (hasAuthParams) {
      // Process OAuth callback and show dashboard
      showLoading('Completing authentication...')

      const callbackAddress = await handleAuthCallback()
      if (callbackAddress) {
        currentAddress = callbackAddress
        hideLoading()
    ...
    // No session found, show login view
    showConnectView()
