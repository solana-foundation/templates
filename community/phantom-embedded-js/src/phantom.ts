...
export async function connect(provider: 'google' | 'apple' = 'google'): Promise<string> {
  try {
    ...
    const result: any = await sdkInstance.connect({ provider })

    // Extract Solana address from result
    if (result?.addresses && Array.isArray(result.addresses)) {
      const solanaAddr = result.addresses.find(
        (addr: any) => addr.addressType === 'Solana' || addr.addressType === 'solana',
      )
...
export async function handleAuthCallback(): Promise<string | null> {
  try {
    ...
    try {
      ...
      const result: any = await sdkInstance.connect({ provider: 'phantom' })
