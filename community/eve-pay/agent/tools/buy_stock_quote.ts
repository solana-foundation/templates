...
export default defineTool({
  ...
  async execute(approved, ctx) {
    ...
    for (const field of ['amount', 'currencyMint', 'decimals', 'network', 'recipient', 'ticker'] as const) {
      if (current[field] !== approved[field]) {
        throw new Error(`Payment terms changed at ${field}; no payment was made. Inspect the quote again.`)
      }
    }
    ...
    const result = await sandbox.run({
      command: `npx --yes @solana/pay@1.0.20 --no-dna --sandbox fetch ${JSON.stringify(url)}`,
    })

    if (result.exitCode !== 0) {
      throw new Error(`Pay.sh exited with code ${result.exitCode}: ${result.stderr.trim()}`)
    }
    ...
    try {
      ...
      // Some paid APIs return text. Keeping it as text is still JSON-serializable.
    }
