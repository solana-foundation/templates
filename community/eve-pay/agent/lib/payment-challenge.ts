...
export async function inspectPaymentQuote(ticker: string, signal?: AbortSignal): Promise<PaymentQuote> {
  ...
  if (response.status !== 402) {
    throw new Error(`Expected an HTTP 402 payment challenge, received ${response.status}`)
  }

  const authenticate = response.headers.get('www-authenticate')
  const encodedRequest = authenticate?.match(/(?:^|,\s*)request="([^"]+)"/)?.[1]
  if (!encodedRequest) {
    throw new Error('The endpoint did not return a supported MPP payment challenge')
  }
